...
class FaultSemanticBuilder:
    ...
    # synthesize_compound_semantics MODIFIED
    def synthesize_compound_semantics(self, single_fault_prototypes):
        """
        Synthesizes compound fault semantics using L2 normalization of components,
        summation, and final L2 normalization. This aims to reduce similarity
        compared to the element-wise maximum approach.
        """
        ...
        compound_semantics = {}
        # Define which single faults make up each compound fault
        ...
        print(
            f"Synthesizing compound semantics using normalized sum approach (Latent Dim: {self.actual_latent_dim})...")

        for compound_type, components in compound_combinations.items():
            component_semantics_normalized = []
            all_components_found = True
            found_component_names = []  # Keep track of which components were actually found

            for comp_name in components:
                # Retrieve the prototype vector for the single fault component
                proto = single_fault_prototypes.get(comp_name)

                # Validate the retrieved prototype
                if proto is not None and np.all(np.isfinite(proto)) and len(proto) == self.actual_latent_dim:
                    # L2 Normalize the component vector before combining
                    norm = np.linalg.norm(proto)
                    if norm > 1e-9:  # Avoid division by zero for zero vectors
                        normalized_proto = proto / norm
                        component_semantics_normalized.append(normalized_proto)
                        found_component_names.append(comp_name)
                    else:
                        # Handle zero vectors - append as is (it won't affect sum)
                        component_semantics_normalized.append(proto)
                        found_component_names.append(comp_name)
                        # print(f"W: Component '{comp_name}' has zero norm during synthesis for '{compound_type}'.") # Optional Warning
                else:
                    # If any component is missing or invalid, we cannot synthesize this compound type
                    all_components_found = False
                    print(
                        f"W: Cannot synthesize '{compound_type}': Missing or invalid component '{comp_name}'. Prototype: {proto}")
                    break  # Stop processing components for this compound type

            # Only proceed if all necessary components were found and are valid,
            # and we actually collected some normalized vectors
            if all_components_found and component_semantics_normalized:
                # Sum the L2-normalized component vectors element-wise
                synthesized_sum = np.sum(component_semantics_normalized, axis=0)

                # L2 Normalize the final summed vector
                final_norm = np.linalg.norm(synthesized_sum)
                if final_norm > 1e-9 and np.all(np.isfinite(synthesized_sum)):
                    synthesized_final = synthesized_sum / final_norm
                    # Final check for safety
                    if np.all(np.isfinite(synthesized_final)):
                        compound_semantics[compound_type] = synthesized_final
                        # print(f"  Successfully synthesized '{compound_type}' from {found_component_names}.") # Optional Verbose Log
                    else:
                        print(
                            f"W: Final synthesized vector for '{compound_type}' became non-finite after normalization. Skipping.")
                elif np.all(np.isfinite(synthesized_sum)):  # Handle case where sum resulted in zero vector
                    # print(f"W: Synthesized sum for '{compound_type}' resulted in zero vector. Setting to zero.") # Optional Warning
                    compound_semantics[compound_type] = np.zeros(self.actual_latent_dim, dtype=np.float32)
                else:
                    print(
                        f"W: Synthesized sum for '{compound_type}' was non-finite before final normalization. Skipping.")

            # else: # Optional: Log why synthesis failed if needed
            #     if not component_semantics_normalized and all_components_found:
            #          print(f"W: Could not synthesize '{compound_type}' because normalized component list was empty (e.g., all zero vectors?).")

        if not compound_semantics:
            print("W: No compound semantics were successfully synthesized.")
        # else: # Optional Summary Log
        # print(f"  Finished synthesis. Generated {len(compound_semantics)} compound types: {list(compound_semantics.keys())}")

        return compound_semantics

    def tensor_fusion_combine(self, vectors):
        """
        使用张量积进行交互式语义合成，再投影回 latent 空间维度。
        """
        if len(vectors) < 2:
            return vectors[0]
        result = vectors[0]
        for v in vectors[1:]:
            result = np.outer(result, v).flatten()  # Outer product + flatten
            # 映射回 AE latent 维度（压缩为与 component vector 相同维度）
            result = result[:self.actual_latent_dim] if len(result) > self.actual_latent_dim else np.pad(result, (
            0, self.actual_latent_dim - len(result)))
        return result / (np.linalg.norm(result) + 1e-8)  # L2归一化
...
class ZeroShotCompoundFaultDiagnosis:
    ...
    def train_dual_channel_cnn(self, data_dict, semantic_dict, epochs=CNN_EPOCHS, lr=CNN_LR):
        ...
        for epoch in range(epochs):
            ...
            for inputs, labels in train_loader:
                 ...
                 w_ce, w_contr, w_consist = 1.0, 0.1, 0.1 # Loss weights
    ...
    def run_pipeline(self):
        ...
        try:
            ...
            self.visualize_data_semantics_distribution(data_dict) # Pass only data_dict

            print("\n--- Step 4: Train CNN ---")
            self.train_dual_channel_cnn(data_dict, semantic_dict)
            if self.cnn_model is None: raise RuntimeError("CNN training failed")

            print("\n--- Step 5: Train SEN ---")
            self.train_semantic_embedding(semantic_dict, data_dict)
            if self.embedding_net is None: raise RuntimeError("SEN training failed")
...
if __name__ == "__main__":
    set_seed(42)
    data_path = "E:/研究生/CNN/HDU1000-600" # <--- !!! MODIFY THIS PATH !!!

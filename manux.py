...
class ZeroShotCompoundFaultDiagnosis:
    ...
    def load_data(self):
        ...
        for fault_type, file_name in fault_files.items():
            ...
            print(f"Loading {fault_type} from {file_name}...")
            if not os.path.exists(file_path):
                print(f"W: File not found: {file_path}. Skipping.")
                continue

            try:
                mat_data = sio.loadmat(file_path)
                signal_data_raw = None
                potential_keys = [k for k in mat_data if
                                  not k.startswith('__') and isinstance(mat_data[k], np.ndarray) and mat_data[
                                      k].size > 500]
                if potential_keys:
                    signal_data_raw = mat_data[potential_keys[-1]]
                if signal_data_raw is None:
                    print(f"E: No suitable data in {file_name}. Skip.")
                    continue
                ...
                # 存储原始信号
                raw_signals_by_type[fault_type] = signal_data_flat
                print(f"  Loaded {fault_type}: {len(signal_data_flat)} samples")

            except Exception as e:
                print(f"E: Error loading {file_name}: {e}")
                traceback.print_exc()

        if not raw_signals_by_type:
            print("E: No raw signals loaded.")
            return None

        # 2. 分离单一故障和复合故障信号
        single_fault_signals = {k: v for k, v in raw_signals_by_type.items() if k in single_fault_keys}
        compound_fault_signals = {k: v for k, v in raw_signals_by_type.items() if k not in single_fault_keys}

        if not single_fault_signals:
            print("E: No single fault signals for training.")
            return None

        # 3. 将单一故障原始信号划分为训练集和验证集
        train_signals = {}
        val_signals = {}

        for fault_type, signal in single_fault_signals.items():
            # 将每种故障的信号分成多个子段，然后进行训练/验证分割
            # 这是为了保持连续信号的完整性，同时允许分层抽样
            n_chunks = max(5, min(20, len(signal) // 10000))  # 根据信号长度决定分块数量
            chunk_size = len(signal) // n_chunks
            signal_chunks = []

            for i in range(n_chunks):
                start_idx = i * chunk_size
                end_idx = start_idx + chunk_size if i < n_chunks - 1 else len(signal)
                signal_chunks.append(signal[start_idx:end_idx])

            # 分层抽样，确保训练集和验证集中有相同比例的每种故障
            train_chunks, val_chunks = train_test_split(
                signal_chunks, test_size=0.3, random_state=42
            )

            if train_chunks:
                train_signals[fault_type] = np.concatenate(train_chunks)
            if val_chunks:
                val_signals[fault_type] = np.concatenate(val_chunks)
        ...
        # 处理训练集信号
        for fault_type, signal in train_signals.items():
            label_idx = self.fault_types[fault_type]
            processed_segments = self.preprocessor.preprocess(signal, augmentation=True)  # 仅为训练集启用增强

            if processed_segments is None or processed_segments.shape[0] == 0:
                print(f"W: Preprocessing yielded no segments for {fault_type} in training set.")
                continue

            X_train.append(processed_segments)
            y_train.extend([label_idx] * len(processed_segments))
            print(f"  Processed {fault_type} (train): {len(processed_segments)} segments.")

        # 处理验证集信号
        for fault_type, signal in val_signals.items():
            label_idx = self.fault_types[fault_type]
            processed_segments = self.preprocessor.preprocess(signal, augmentation=False)  # 验证集不使用增强

            if processed_segments is None or processed_segments.shape[0] == 0:
                print(f"W: Preprocessing yielded no segments for {fault_type} in validation set.")
                continue

            X_val.append(processed_segments)
            y_val.extend([label_idx] * len(processed_segments))
            print(f"  Processed {fault_type} (val): {len(processed_segments)} segments.")

        # 处理测试集（复合故障）信号
        for fault_type, signal in compound_fault_signals.items():
            label_idx = self.fault_types[fault_type]
            processed_segments = self.preprocessor.preprocess(signal, augmentation=False)  # 测试集不使用增强

            if processed_segments is None or processed_segments.shape[0] == 0:
                print(f"W: Preprocessing yielded no segments for {fault_type} in test set.")
                continue

            X_test.append(processed_segments)
            y_test.extend([label_idx] * len(processed_segments))
            print(f"  Processed {fault_type} (test): {len(processed_segments)} segments.")

        # 整合数据
        X_train = np.vstack(X_train) if X_train else np.empty((0, self.sample_length))
        y_train = np.array(y_train)
        X_val = np.vstack(X_val) if X_val else np.empty((0, self.sample_length))
        y_val = np.array(y_val)
        X_test = np.vstack(X_test) if X_test else np.empty((0, self.sample_length))
        y_test = np.array(y_test)

        print(f"Final dataset: {len(X_train)} train, {len(X_val)} val, {len(X_test)} test samples")
